...
def prepare_gainers_df(index: str, view_name: Optional[str]) -> Optional[pd.DataFrame]:
    metrics = APP_SETTINGS["commands"]["screen_gainers"]["metrics"]
    companies_data = investor8_sdk.ScreenerApi().get_top_stocks("winners", index=index)
...
def gainers(index: str, view_name: Optional[str], export_path: Optional[str]) -> None:
    ...
    with console.status("Fetching data...", spinner="material"):
        df = prepare_gainers_df(index, view_name)
    if df is None:
    ...
    if export_path:
        if export_path.split(".")[-1] == "html":
            ...
            table = df2Table(
                prepare_current_metrics_formatted_df(df, "console").sort_values("Change", ascending=False),
                columns_justify=columns_justify,
            )
            export_to_html(table, export_path)
            return
        export_data(
            prepare_current_metrics_formatted_df(df, "store").sort_values("Change", ascending=False),
            export_path,
            column_width=18,
