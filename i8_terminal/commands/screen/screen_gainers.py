...
def prepare_gainers_df(view_name: Optional[str]) -> Optional[pd.DataFrame]:
    companies = investor8_sdk.ScreenerApi().get_top_stocks("winners")
...
@pass_command
def gainers(view_name: Optional[str]) -> None:
    """
    ...
    with console.status("Fetching data...", spinner="material"):
        df = prepare_gainers_df(view_name)
    table = df2Table(df)
    console.print(table)
